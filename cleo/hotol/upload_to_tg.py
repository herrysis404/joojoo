...
from cleo.hotol.display_progress import progress_for_pyrogram, humanbytes
from cleo.hotol.help_Nekmo_ffmpeg import take_screen_shot
from cleo.hotol.split_large_files import split_large_files
from cleo.hotol.copy_similar_file import copy_file
    ...
    if os.path.isdir(local_file_name):
        ...
        if os.path.getsize(local_file_name) > TG_MAX_FILE_SIZE:
            ...
            i_m_s_g = await message.reply_text(
                "Telegram does not support uploading this file.\nMax file 2000 MiB or 1.9531 GiB per file\n"
    ...
    if os.path.isfile(file_upload):
       ...
       # button.append([pyrogram.InlineKeyboardButton(text="☁️ CloudUrl ☁️", url=f"{gau_link}")])
        if INDEX_LINK:
            ...
            button.append([pyrogram.InlineKeyboardButton(text="☁️   Visit   ☁️", url=f"{tam_link}")])
        button_markup = pyrogram.InlineKeyboardMarkup(button)
        await asyncio.sleep(EDIT_SLEEP_TIME_OUT)
        await messa_ge.reply_text(f"💎  {file_upload} has been Uploaded successfully  💎 <a href='tg://user?id={g_id}'>🤒</a>\n📀 Size: {gjay}", reply_markup=button_markup)
