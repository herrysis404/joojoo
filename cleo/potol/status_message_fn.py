...
from cleo.hotol.download_aria_p_n import call_apropriate_function, aria_start
from cleo.hotol.upload_to_tg import upload_to_tg


from cleo.hotol.display_progress import (
    get_readable_time,
    humanbytes
)
...
async def status_message_f(client, message):
    ...
    for download in downloads:
        ...
        try:
            ...
            #if progress_percent_string == "100.00%":
             #continue
        except:
            pass
    ...
    if msg == "":
        msg = "Dont add file size more than free space  🧔🏻🧔🏻🧔🏻!"
  ...
  #  currentTime = TimeFormatter((time.time() - BOT_START_TIME))
   ...
   # currentTime = time.asctime( time.localtime(time.time()) )
    ...
    ms_g = f"<b>Server Uptime</b>: <code>{currentTime}</code>\n" \
